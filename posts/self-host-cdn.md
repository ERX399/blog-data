...
    用户 --> 520pro.top --> |DNS解析|cdn[oraclecdn.520pro.top] --> 1[Oracle1]
...
https-redirect = true
https-redirect-host = "520pro.top"
https-redirect-from-port = 80
https-redirect-from-hosts = "520pro.top"
...
```bash
./acme.sh --issue --dns dns_cf -d 520pro.top -d '*.520pro.top'
```
...
```bash
acme.sh --install-cert -d 520pro.top \
...
将两个甲骨文IP写入 `oraclecdn.520pro.top` 中

![](/img/self-host-cdn-self-host-cdn-3.png)

接着将 `520pro.top` CNAME `oracle.520pro.top` 
