...
如果某个 `pathname` 没有命中任何规则，也会统一落到默认回退源，这样就能兼容类似 `https://520pro.top/posts/pin/ --> https://blog.acofork.com/posts/pin/` 这样的场景。
...
## 在哪搞个短链
我的 `520pro.top` 是在 https://porkbun.com 购买的，价格大概是一年一百元左右。其他后缀也可以考虑，比如 `.im`、`.mk`。
...
| `BASE_DOMAIN` | `你的短链域名` | 例如 `u.520pro.top` 或 Worker 的默认域名 `xxx.workers.dev` |
