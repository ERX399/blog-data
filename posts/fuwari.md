  ...
  - icon：你可以前往 [icones.js](https://icones.js.org/) 搜索想要的图标。例如 QQ 可以填写 `fa6-brands:qq`。Fuwari 默认支持 `fa6-brands`、`fa6-regular`、`fa6-solid`、`material-symbols` 等类型，必要时也可以在 `astro.config.mjs` 中继续扩展。
   ...
   - ```ts
     ...
     export const siteConfig: SiteConfig = {
       ...
       banner: {
         enable: true,
         src: 'https://eo-r2.520pro.top/myblog/img/222.webp',   // Relative to the /src directory. Relative to the /public directory if it starts with '/'
     ...
     export const profileConfig: ProfileConfig = {
       avatar: 'https://eo-r2.520pro.top/myblog/img/111.webp',  // Relative to the /src directory. Relative to the /public directory if it starts with '/'
       name: '夏之',
...
4. ```markdown
   ...
   - image：文章封面图（同目录需要写 `./` 如：`./https://eo-r2.520pro.top/myblog/img/2024-10-14-11-33-28-image.webp`）
...
7. 这并不复杂。借助 MarkText，可以像使用传统博客编辑器一样直接复制粘贴图片，不过需要先做一点简单配置：
  ...
  - 这样一来，插入图片时就会先把图片复制到 `https://eo-r2.520pro.top/myblog/img` 对应的位置，再自动以 `![1](https://eo-r2.520pro.top/myblog/img/1.webp)` 这样的格式写入 Markdown 文件。你只需要复制粘贴，剩下的步骤 MarkText 会自动完成。
