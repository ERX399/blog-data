---
title: 从 SPA 到全站 SSR：520pro.top 的服务端渲染实践录
description: 上篇 SEO 的文章里我提到已经把网站从 Next.js 迁到了 Vite+React Router 7 的纯 SPA，并靠边缘
  ...
  不是边缘补丁能解决的。这篇文章记录 520pro.top 从 SPA 再次迁移到 React Router 7 Framework Mode 全站 SSR
...
无论是旧项目还是新项目，**520pro.top 从来没有用过 SSG**。旧项目是纯 CSR——前端在浏览器里发请求拉博客后端的 `posts.json` 索引，拿到列表后再按需取单篇 markdown 回来渲染。新项目是 SSR+ISR——服务端在请求时读本地文件渲染 HTML，缓存按 `generatedAt` 版本标记失效。
...
RSS 2.0 规范要求封面图同时带 `url`、`length`、`type` 三个属性，缺一个 Search Console 就报「缺少 XML 属性」（本站实测报了 30 处）。而封面图在 `raw-posts.520pro.top`，这个域名跑在 Cloudflare Workers 后面——缓存命中的响应**既不返回 `content-length`，也不支持 `Range: bytes=0-0`**（直接回 200 而非 206），网络侧根本拿不到字节数。
...
GitHub Action deploy 完成
  → POST https://520pro.top/internal/revalidate?token=...
...
把博客从源码里拆出去（[那篇文章](/posts/micro-blog-service)）、给 SPA 加上 SEO（[那篇文章](/posts/svaf-next-seo)）、再到现在全站 SSR——这三次迁移下来，520pro.top 的前端架构终于到了我满意的状态。后面如果有时间，可能会再写一篇关于整个后端体系的（论坛 D1 → SQLite + 生图 nDI + Cloudflare 组网），那个坑更多。
