...
简单来说，就是博客站叫 blog.520pro.top，如果你还有论坛，那就叫 from.520pro.top，如果你还有其他服务，比如一些在线工具，就叫 tools.520pro.top。
...
诚然，我们首先需要一个前端。前端肯定是用户访问越简单越好，所以就用短域名。那么前端就定好了，用 `520pro.top` 

![image.png](/img/image-11.png)

那么后端呢？由于用户几乎是不可知的，只有少数开发者会打开 F12 去抓，所以我们后端就来一个很有风味的 API 地址，叫做 `raw-posts.520pro.top`。
...
如果你让订阅器去读取后端的 [raw-posts.520pro.top/rss.xml](http://raw-posts.520pro.top/RSS.XML，就会导致以下问题：)
...
再简单一点来说，它可以做到用户访问 `520pro.top/rss.xml` 就等于访问 `raw-posts.520pro.top/rss.xml` 

![image.png](/img/image-18.png)

这个时候，我们只需要再加一条规则，让我们的渲染器在渲染 RSS 时把链接全部改成我们前端的地址，也就是 [520pro.top](http://520pro.top)，就可以做到完全无感的浏览。用户这个时候如果想要看我的网站，跳转的也是正确的前端页面。
...
2. 域名变更风险：如果说我们后续的 520pro.top 这个域名它不再继续使用了，我们还需要再更改一遍，更改为之后的域名。
...
但脚本有点“用力过猛”：它把正文里**所有**根相对链接（也就是以 `/` 开头的链接）统统前缀成了后端地址。图片没事，可要是我在文章里写了一个站内跳转，指向另一篇文章（`/posts/xxx` 这种根相对链接），它就会被改写成后端的裸地址 `raw-posts.520pro.top/posts/xxx`。

这就又回到了我们讲 RSS 时提到的那个尴尬场面：用户满心欢喜点进去，结果落到的是后端那个只吐裸 Markdown 的地址，没有任何前端渲染，一脸懵。

治本当然是去改预构建脚本，让它只对图片资源做前缀，站内链接要么保持相对、要么前缀成前端域名 `520pro.top`。不过我这次偷了个懒，用 Cloudflare 的重定向规则做了个兜底：只要是**浏览器**访问后端的 `/posts/*`（且不是 `.md` 结尾的原文请求），就 302 弹回前端的同名路径。

匹配表达式长这样：

```
(http.host eq "raw-posts.520pro.top" and starts_with(http.request.uri.path, "/posts/") and not ends_with(http.request.uri.path, ".md") and http.user_agent contains "Mozilla")
```

目标地址用动态表达式 `concat("https://520pro.top", http.request.uri.path)` 就行。
